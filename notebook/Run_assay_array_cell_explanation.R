...
saveRDS(fgsea, file = "log/array_ccle_raw_svd_deg_gsea_kegg.rds")
write.table(fgsea[,1:5], file = "log/array_ccle_fgsea.xls", sep = "\t", col.names = T, row.names = F)


# stromal immune score
stroImm.res = stroImm(ralign)
saveRDS(stroImm.res, file = "log/array_ccle_raw_svd_stroImm.rds")
...
colnames(out.table) = c("id", "raw_stromal_score", "raw_immune_score",
                        ...
                        "correct_purity")
write.table(out.table, file = "log/array_cell_stroImm.xls",sep = "\t", row.names = F, col.names = T)
...
# ann$lineage[which(ann$max_proportion<0.5)]= "mixture"
...
saveRDS(seu, file = "log/array_ccle_svd_seu.rds")
...
saveRDS(meta, file = "log/array_ccle_cluster_meta.rds")
write.table(meta, file = "log/array_ccle_cluster_meta.xls", sep = "\t", row.names = F, col.names = T)

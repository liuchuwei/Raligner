...
# load data----
bulk = readRDS("data/tcga_dat.rds")
ccle = readRDS("data/ccle_dat.rds")

# create raligner project----
bulk_ann = bulk$bulk_ann %>% data.frame()
row.names(bulk_ann) = bulk_ann$sampleID
bulk_dat = bulk$bulk_dat[, row.names(bulk_ann)]
...
saveRDS(ralign, file = "log/tcga_ccle.rds")
